...
export default function ShowInfo({ callback, goBack }) {
  ...
  const [averageColor, setImageUri] = useAverageColor(
    activeTrack.imageUri?.uri,
    '#1B1B1F',
  );
  ...
  useEffect(() => {
    let newStageUsers = [
      { ...user, isMuted: isMuted, key: user.username },
      ...remoteUsers.map((participant, i) => {
        return {
          ...participant.user,
  ...
  return (
    <ImageBackground
      ...
      style={[
        styles.image,
        { backgroundColor: activeTrack.uri ? 'transparent' : colorStyles.card },
      ]}
    >
