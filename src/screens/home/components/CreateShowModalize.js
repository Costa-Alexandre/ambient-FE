...
export default function CreateShowModalize({
  ...
  const createShow = async () => {
    const newShow = await postShow({
      ...
      creator_id: JSON.stringify(user._id).replace(/\"/g, ''),
      date_created: JSON.stringify(new Date()).replace(/\"/g, ''),
      date_scheduled: JSON.stringify(new Date()).replace(/\"/g, ''),
    });
    close();
    callback(newShow);
  };
